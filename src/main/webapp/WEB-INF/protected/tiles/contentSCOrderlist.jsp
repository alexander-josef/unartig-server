    ...
    <div class="contentW rightalign padding19top">
        <c:if test="${shoppingCart.backToAlbumParams!=null}">
            ...
            <div class="leftalign">
                ...
                <html:link action="/album/${albumBean.album.genericLevelId}/${albumBean.album.longTitle}" name="shoppingCart" property="backToAlbumParams">
            ...
            <ul class="album">
                <li class="albumslide">
                    ...
                    <div class="album_slide_info_sc">
                        ...
                        <html:link action="/removeFromCart" paramId="<%=Registry._NAME_ORDERED_PHOTO_ID_PARAM%>" paramName="scItem" paramProperty="photoId">
                    ...
                    <td class="sc_format_secondcol">
                        ...
                        <c:if test="${scItem.product.digitalProduct}">
                            <html:link action="/explainDigital" >
                                <html:img src="/images/questionmark.gif" alt="help" onclick="PriceList=window.open('/explainDigital.html','Preisliste','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=400px,height=450px,left=200px,top=100px'); PriceList.focus(); return false;"/>
                            </html:link>
                        </c:if>
                    </td>
                    <td class="sc_format_thirdcol">
                        ...
                        <c:if test="${scItem.digitalOrderItem}">
                            <html:text name="scItem" property="quantity" indexed="true" size="3" styleClass="txt" disabled="true" value="1" titleKey="shoppingcart.amount.digital.title" bundle="CONTENT"/>
                        </c:if>
                        <c:if test="${!scItem.digitalOrderItem}">
        ...
        <c:if test="${scItemIndex % 4 == 3}">
            ...
            <div class="rightalign padding19top">
                <%--<html:link href="javascript:document.forms[0].submit()" styleId="recalculate">--%>
                    <html:img bundle="IMAGES" srcKey="btn.recalculate.src" altKey="btn.recalculate.alt" onclick="postSimpleForm('update')"/>
                <%--</html:link>--%>
            </div>
...
-->
            ...
            updateShoppingCart??
            -->
                <html:image bundle="IMAGES" srcKey="btn.checkout.src" altKey="btn.checkout.alt" onclick="postSimpleForm('checkOut')"/>
        </div>
    </div>
